...
cmake -DZMQ_LOCAL_BUILD=ON  -DENABLE_SWIG=ON  -DCMAKE_BUILD_TYPE=Release -A x64 -DBUILD_HELICS_TESTS=OFF -DCMAKE_PREFIX_PATH="%LIBRARY_PREFIX%" -DCMAKE_INSTALL_PREFIX="%LIBRARY_PREFIX%" %BUILD_PYTHON%" -DPYTHON_INCLUDE_DIR=%HELICS_PYTHON_INCLUDE_DIR% -DPYTHON_LIBRARY=%HELICS_PYTHON_LIBRARY% -DPYTHON_EXECUTABLE=%HELICS_PYTHON_EXECUTABLE% ..
